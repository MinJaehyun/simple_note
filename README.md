...
- (ing) 삭제 누르면, 삭제한 내용은 휴지통에 들어가 있어야 한다. (휴지통에 내용은 더 이상 삭제되지 않는다)
0. 휴지통 페이지 구성: (상단: (simple note, 정렬 기능), 중단: 검색창, 하단: 모든 메모)
1. ~~휴지통 페이지 우선 만들기: 이름: trash_can~~
2. ~~중단 메모 리스트 나타내기 ()
3. 삭제 버튼 찾기
4. 쇼핑 범주에 담는 메모를 삭제하면, 휴지통에 담긴 메모에는 쇼핑 범주가 그대로 담겨있다. 그런데 쇼핑 범주를 삭제하면 관련된 모든 메모(휴지통 포함)는 미분류 로 정해져야 한다. 범주를 삭제 시, "관련 범주 안에 모든 메모는 미분류로 지정 됩니다" 문구 나타내기
5. 휴지통에서 내용이나 범주 변경해도 휴지통에 그대로 있다.
