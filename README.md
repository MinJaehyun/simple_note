...
- (대기) 코드 리펙토링 필요하다. 공통 영역 처리하고, 4~500이상인 부분 처리하기. 위에서 부터 한개씩 처리하기
- (대기) 메모 내용 음성으로 읽기: openai api 있음
- (대기) 위와 반대로 음성을 녹음해서 text로 작성해주는 기능: openai api 있음
- (대기) 위 처리하기 전, 메모장 킨 상태에서 실기기 흔들면(shake 기능 추가) 즉시 녹음되는 기능으로 접근하게 만들기 (이를 통해 간편성 높임)
- 이미지 확대 축소 기능 구현 (add_memo, update_memo): 추후 중복 코드 리펙토링 하기
- ai 퀴즈 기능 구현 미정
- 삭제하면 삭제된 순서대로 들어가게 기본적으로 만들기 (정렬 기능 누르면, 삭제 순서대로 표시하기 목록 만들기)
- 추후 협업 기능 (팀원과의 정보 공유가 용이하여 협업 시 유용하다. 공동 작업을 통해 아이디어를 발전시키고, 의견을 교환할 수 있다)
- AI 기반 추천 기능 
  AI가 사용자의 행동을 분석(메모장의 내용)하여 유용한 정보를 추천(dialog)합니다.
  새로운 아이디어나 자료를 발견하는 데 도움을 줄 수 있습니다.
  이러한 특성들 덕분에 AI 메모장은 개인의 생산성을 높이고, 업무 및 일상에서의 효율성을 크게 향상시킬 수 있는 도구입니다.
...
## 2024-08-05
### Features
- ~~setting controller 코드 개선 작업~~
- ~~달력 기능 개선: 이전 월의 날짜를 클릭할 때 현재 월로 자동으로 이동하지 않고 클릭한 월을 유지하기위해, focusedDay 값을 사용하지 않고, 대신 클릭한 날짜에 해당하는 월로 포커스를 유지하도록 설정해야 합니다~~


## 2024-08-05 (여기서부터 앱 업데이트 내용에 새로 추가할 것들)
### Features
- ~~문구 수정: "앱을 삭제해도 재설치 시에 기존 데이터를 그대로 가져오고 있습니다"~~
- ~~setting controller -> repo 리펙토링~~


## 2024-08-03
### Bug Fixes
- ~~휴지통의 복원된 메모를 달력에서 내용 변경 시, 중복 생성되는 오류 해결: 정렬 함수 호출하여 휴지통에서 복원한 메모의 인덱스가 뒤엉키지 않게 설정함.~~
- ~~범주가 설정된 메모를 삭제 시, 휴지통의 메모의 범주가 제거되지 않는 문제 해결: 범주가 설정된 메모를 삭제 시, 메모의 범주는 제거되고 휴지통으로 이동되도록 처리함. (update_memo, popup btn)~~ 
- ~~범주에서 각각의 범주를 삭제하면 다른 요소가 삭제되는 문제점 해결: popup에서 삭제 누르면 해당 범주가 제거되고, 관련된 범주의 메모는 '미분류'로 지정되도록 처리함.~~
- ~~범주 내용 변경 시, 관련 범주의 메모가 변경한 범주로 변경되지 않는 문제점: 삼항 연산자로 처리함.~~
- ~~범주 위,아래 reorder 이동 후, 범주를 변경하면 관련 범주를 나타내지 못하는 문제점: reorder 후, 범주 로드하여 재 정렬함.~~
...
- ~~아이콘 개선~~
### Bug Fixes
- ~~휴지통이 빈 경우 화면을 나타내지 못하는 에러 발생: 의존성 주입 문제 lazy 코드를 해결~~


## 2024-07-30 
### Features
- ~~버전 업데이트~~
- ~~휴지통에서 복원한 메모의 체크기능을 달력에서 사용하면 이제 잘 기능한다.~~
- ~~text to icon of create memo or category btn~~
### Bug Fixes
- ~~휴지통에서 복원한 메모에 들어가서 내용 변경하면 중복된 새로운 내용이 나타난다.~~
...
## 2024-07-26 (여기서부터 앱 업데이트 내용에 새로 추가할 것들)
### Features 
- ~~크리티컬 이슈: 범주 내에 메모의 체크 클릭 시, 새로운 메모가 중복 생성되는 index 에러 해결~~
- ~~하단 화면 절반 정도 넓혀서, 아래 메모가 있다는 것을 사용자가 알 수 있게 처리~~
- ~~리펙토링: memo_controller의 메모를 추가,변경,삭제한 후 리스트에 직접 적용하여 불필요한 데이터 로딩을 피함 (메모가 많아지면 이 방식이 효율적)~~
- ~~controller & repository: 코드 개선: ~~memo~~, ~~category, trash_can_memo, settings
...
- ~~필수: 범주 클릭하면 관련 범주가 선택한 메모장으로 이동하게 만들기~~
- ~~ㄴ "모든"  클릭 시, 메모장 페이지에 "모든" 범주의 메모를 나타낸다.~~
- ~~ㄴㄴ 메모장 페이지로 이동하고, 이동할 때 "모든" 범주가 선택되어야 하므로, 인자 내려준다 ((Get.offAll("모든")))~~ 
- ~~ㄴ "미분류" 클릭 시, 메모장 페이지에 "미분류" 범주의 메모를 나타낸다 (Get.offAll("미분류"))~~
- ~~ㄴ "임의 범주" 클릭 시, 메모장 페이지에 "임의 범주" 범주의 메모를 나타낸다 (Get.offAll("?"))~~ 
- ~~ㄴㄴ 괄호 안에 임의의 인자 넣는 방법?? (선택한 범주를 넣으면 된다)~~
...
## 2024-07-20 (여기서부터 앱 업데이트 내용에 새로 추가할 것들)
### Features
...
## 2024-07-19
### Features
- ~~카드 height를 media 크기별로 알맞게 지정하기 (총 5개: 메모장 등등)~~
- ~~리펙토링: 155 to 75 라인 개선: footer_navigation_bar_widget~~
- ~~ai검색창에 엔터 이벤트 기능 구현~~
- ~~정렬 버튼 아닌, ListTile 선택해도 버튼이 선택되도록 기능 개선~~
- ~~메모 알람 기능 삭제 및 백업/복원 기능 후순으로 지정하기~~
- ~~메모장: 글자 굵기 및 크기 미분류와 동이랗게 설정하기~~
- if, elfe 검색하고, 개선할 부분 찾기 
### Bug Fixes


## 2024-07-18
### Features
- ~~달력 및 범주 및 메모장 및 휴지통의 수정, 삭제, 복원, 완전히 삭제 기능에 아이콘 및 색상(빨강색) 설정~~
...
## 2024-07-09 
### Features
- ~~(1/3 기획) 메모장: ai 만들기 (add_memo, update_memo, 휴지통은 적용하지 않는다)~~
