...
## 2024-06-28
### Bug Fixes: 1일 1버그 찾기 
- 메모장: 라이트 모드는 변경한 폰트가 적용 되는데, 다크 모드는 변경한 폰트가 적용 안된다.
- ㄴ ?
...
- (진행 중) 버전 1.0.2+9 버전에 앱이 다운받은 앱에는 1.0.0으로 나타나는 듯하다. 왜 그런지??
- (진행 중) (2/2) 배포 후 테스트 하기: 앱 아이콘 기본 플루터 아이콘을 내 프로젝트 아이콘으로 변경하기 (런처 아이콘 생성)
- ㄴ $ flutter build appbundle 실행한 파일이 변경한 파일이 맞는지 부터 확인하기  
- ㄴ 앱 1.0.2+9 
