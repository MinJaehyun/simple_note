...
## todo
- 와이어프레임 변경된 점 수정하기
- (ing) 이전에 프로젝트 진행 계획 가져오기 (트렐로)
- commit 번호 링크 달기
- 메모 생성 기능 개선하기(입력칸 늘리기: add 메모(maxLines: 6))
- 추후: 작성된 메모는 최신 순으로 정렬하여 UI에 나타내기 
- 추후: 카테고리 내에 crud 위젯으로 분리하기
- 추후: 홈페이지에 흐릿하게 움직이는 배경 넣기
- fixme: navigation bar 아이콘 걸친 영역 클릭하면 아이콘은 작동하지만, 페이지 띄우지 않는다 
- fixme: 달력 초기 진입 시, 생성한 날짜에 모든 메모를 분홍 마킹으로 내타내야 한다
- 
